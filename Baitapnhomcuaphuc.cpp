...
void danhSachChienDichCoSoNgayItNhat(chien_dich a[], int n);
void tinhTongSoNgayDienRaChienDich(chien_dich a[], int n);
...
void menu_thaotac(chien_dich a[], int n){
    ...
    do
    {
        cout<<"\n\tMENU\n============================\n0. Ket Thuc.\n1. Nhap Danh Sach Chien Dich Thu Cong.(Chinh)\n2. Nhap Danh Sach Chien Dich Tu File Text.(Chinh)\n3. Xuat Danh Sach Chien Dich.(Huy)\n4. Tinh Tong So Tinh Nguyen Vien Tham Gia.(Huy)"<<endl;
        cout<<"5. In Danh Sach 5 Chien Dich Co So Tinh Nguyen Vien Cao Nhat.(Phuc)\n6. Kiem Tra Danh Sach Cac Chien Dich Co Duoc Sap Xep Tang Dan Theo So Ngay Dien Ra.(Phuc)\n7. Sap Xep Danh Sach Giam Dan Theo So Ngay Dien Ra.(Phuc)\n8.In Danh Sach Chien Dich Co Thoi Gian Dien Ra It Nhat.(Vu) \n9.Tinh Tong So Ngay Dien Ra Cua Cac Chien Dich.(Vu) \n10. Tim Chien Dich Bang Ten.(Ky) \n11. Hien Thi Tat Ca Chien Dich Keo Dai 5 Den 10 Ngay.(Ky)"<<endl;
        ...
        case 7:
            ...
            break;
        case 8:
            danhSachChienDichCoSoNgayItNhat(a,n);
            break;
        case 9:
            tinhTongSoNgayDienRaChienDich(a,n);
            break;
        case 10:
...
void danhSachChienDichCoSoNgayItNhat(chien_dich a[], int n)
{
    ...
    for(int i=0; i<n-1;i++)
        for(int j= i+1;j<n;j++)
            {
                if(d[i].ngay_dienra>d[j].ngay_dienra)
                {
                    SWAP(chien_dich, d[i],d[j]);
                }
            }
    int sapxep = 5;
    cout<<"Danh Sach chien dich co thoi gian dien ra it nhat la: "<<endl;
    xuatDanhsach(d,sapxep);
}
void tinhTongSoNgayDienRaChienDich(chien_dich a[], int n)
{
    int TongSN=0;
    for(int i=0;i<n;i++)
    {
        TongSN+=a[i].ngay_dienra;
    }
    cout<<"\nTong Thoi Gian Dien Ra Chien Dich La: "<<TongSN;
}
